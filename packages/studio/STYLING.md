...
- A part whose text the reader must copy, and that offers no other way to copy it, opts back in on that part alone, from its own surface document: `userSelect: "text"`, or `"all"` for a token that is only useful whole (the GitHub device code). Today that is the dialog message and the island's own prose, the device code, About's values and package rows, the Assistant's transcript bodies, and the Problems and Activity logs.
- **An opt-in names a part, never a mount point.** `user-select` inherits, so a rule on a box that hosts other documents reopens selection inside all of them: the dialog's `[part="island"]` is the message's richer form AND where the Open Grid picker and Push Schema mount their surfaces, so the rule there reaches only the paragraphs and text-level spans a lit body writes, not the headings and rows a mounted document draws.

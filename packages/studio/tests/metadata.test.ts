...
describe("css-meta.json", () => {
  ...
  /* The `$icons` values are kit MANIFEST names (ui.md §8, one key space), drawn by the Style
     ...
     - wrap: `arrow-u-down-left`, the conventional wrap arrow (along, down, back). `flip-vertical`
       was the Spectrum-era pick, and Phosphor draws it as two mirrored triangles.
     - justify and align: the object-alignment family, which is what Phosphor's `align-*` are (a
       rule and two bars, not text lines). space-between is arrows pushed OUT from a centre rule;
       space-around is its mirror, the weaker of the pair, because Phosphor has no distribute
       family; space-evenly is a plain double arrow, weaker still, chosen because it is distinct
       from the other two in the same row. stretch is the vertical out-arrows, filling the cross
       axis. baseline is a letter on a full-width rule, `text-a-underline`, because
       `text-subscript` is the rich-text bar's Subscript. Every button carries its CSS value as
       its accessible name and tooltip, so a weak glyph is never the only way to tell. */

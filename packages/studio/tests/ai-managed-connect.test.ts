...
describe("connect", () => {
  ...
  test("a connect the backend does not honour says so instead of repainting itself", async () => {
    ...
    expect(part(container, "failure")?.textContent).toContain(
  ...
  test("a passed deadline gets its own words; a cancellation and a redirect get none", async () => {
    ...
    pointer(connectControl(container), "click");
    await flush(6);
    expect(part(container, "failure")?.textContent).toContain("didn't finish");

    // A closed popup is not an error — the user already knows what they did.
    outcome = { status: "canceled" };
    pointer(connectControl(container), "click");
    await flush(6);
    expect(part(container, "failure")).toBeNull();
  ...
  test("a dismissed picker leaves the connection named as unfinished, not as working", async () => {
    ...
    expect(part(container, "failure")?.textContent).toContain("no account is chosen yet");
  ...
  test("surfaces a thrown error and re-enables the button", async () => {
    ...
    expect(part(container, "failure")?.textContent).toContain("popup blocked");
...
describe("the recommendation, and its two moods", () => {
  ...
  test("an account chosen that the backend still refuses says so", async () => {
    ...
    expect(part(container, "failure")?.textContent).toContain("still reports no account chosen");
  });

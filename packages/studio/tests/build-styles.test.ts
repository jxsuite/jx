...
describe("what the generated sheet must contain", () => {
  ...
  test("no Spectrum name survives in ANY stylesheet — the death date came", () => {
    /* This used to assert the opposite of its second half: `styles/spectrum.css` was the one sheet
       that still declared `--spectrum-*`, on `sp-theme` where Spectrum's own ramp was in scope, and
       its header carried a death date. The file is gone, so the assertion is re-aimed at the whole
       directory rather than deleted — a `--spectrum-*` name in any sheet now resolves to nothing,
       silently takes its hex fallback, and stops following the theme. */
    const sheets = [...new Glob("*.css").scanSync(join(ROOT, "styles"))].toSorted();
    expect(sheets.length).toBeGreaterThan(5);
    expect(sheets).not.toContain("spectrum.css");
    for (const sheet of sheets) {
      const text = readFileSync(join(ROOT, "styles", sheet), "utf8");
      expect(text, sheet).not.toContain("--spectrum-");
      expect(text, sheet).not.toContain("--mod-");
      expect(text.replaceAll(/\/\*[\s\S]*?\*\//g, ""), sheet).not.toMatch(/\bsp-[a-z]/);
    }
  });
});

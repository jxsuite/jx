...
import { STRIP_TAGS } from "./to-jx.ts";
...
export async function captureStyles(page: ImportPage): Promise<StyleCaptureResult> {
  return page.evaluate(
    (allowlist: string[], stripTags: string[]) => {
      const stripped = new Set(stripTags);
      ...
      function walk(el: Element, path: number[]) {
        ...
        /* A stripped tag is not walked AND does not consume an index. `to-jx.ts` removes it from
         the tree these paths are later resolved against, so counting it here would shift every
         following sibling — and the whole subtree under it — onto a path that does not exist. */
        let childIdx = 0;
        for (const child of el.children) {
          if (stripped.has(child.tagName.toLowerCase())) {
            continue;
          }
      ...
      /* The sandbox must not set an INHERITED property that is also on the allowlist: the probe
       inherits it and the value becomes that tag's "UA default", so a real declaration equal to it
       is dropped as a no-op by `diffStyles` and the mirror value is emitted on every other node.
       `visibility:hidden` and `pointer-events:none` did exactly that — every genuine
       `visibility:hidden` and `pointer-events:none` on the site was deleted, and `visibility:visible`
       plus `pointerEvents:auto` were stamped onto ~5,300 nodes each, making them the two most
       common declarations in the project. Offscreen positioning alone hides the probes. */
      sandbox.style.cssText = "position:fixed;top:-9999px;left:-9999px;";

...
- **Button rows**: Display, direction, wrapping, alignment and text alignment are rows of buttons, one per common value, each drawn as an icon. A handful of values have no icon that would read as them, such as `nowrap` and `wrap-reverse`, and those buttons carry the CSS keyword itself instead. Hover a button to see the value it writes, click it again to clear it, and open **···** for the values the row has no room for.

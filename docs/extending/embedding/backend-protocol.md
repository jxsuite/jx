...
- **`@jxsuite/ai/gateway`** implements both AI routes for a TypeScript backend, and it is the code the dev server itself runs. `createChatHandler({ resolveUpstream })` answers `ai/chat`: it reads and checks the body, forwards it as an OpenAI-compatible streaming request, and normalizes the provider's stream into the frames above, so a backend inherits the `reasoning`, `usage` and finish rules instead of re-deriving them. The policy stays yours. `resolveUpstream` decides whose key a request uses and which base URL it may reach (the dev server never sends its own environment key to a URL the caller chose, and refuses link-local hosts), and the optional `admit`, `maxBodyBytes`, `maxMessages` and `onAccepted` add rate limits, size limits and an audit hook. A refusal at any of those steps is an `application/problem+json` answer, sent before the provider is contacted. `modelsResponse(body)` answers `ai/models` with the `200` the probe requires, whatever catalogue you pass it. The module imports nothing host-bound, so the same handler runs in a Cloudflare Worker, under Bun or under Node.

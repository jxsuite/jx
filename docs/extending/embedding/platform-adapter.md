...
`readFileBytes` is the read side of that, and it is optional. `readFile` answers a string, which means UTF-8, so every byte that is not part of a valid UTF-8 sequence comes back as a replacement character: a JPEG through `readFile` is not a damaged JPEG, it is a run of U+FFFD that nothing can decode. An adapter that implements `readFileBytes` answers an `ArrayBuffer` instead, and Studio can then decode a project image. If your transport serializes its arguments, send base64 back and decode it with `base64ToBytes` from `@jxsuite/studio/base64`, which is the mirror of the `toBase64` above.

:::doc-note
Serving the file at a URL is not a substitute. Studio can display an image from a URL, but reading its pixels back means drawing it into a canvas, and a canvas that has drawn a cross-origin image is tainted: reading it throws. The desktop adapters are exactly that case, because the canvas origin is a per-window loopback server while the shell document is not. CORS is not the escape hatch either, since the loopback security model depends on the browser refusing cross-origin reads. Bytes handed back through the adapter avoid the question entirely.
:::
...
| ------------------- | --------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Binary reads        | `readFileBytes`                                                                                                             | Studio can only read a project file as text, so any surface that must decode one stays hidden                                                                                 |

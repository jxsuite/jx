...
Density is a second attribute on the root element. `data-density="compact"` steps the body type down one size (`--jx-text-md` to 11px, `--jx-leading-md` to 16px), so more of a label fits a row and a panel reads smaller; `data-density="comfortable"` raises the control height to 28px. The control height itself never goes below 24px at any density: WCAG 2.2 SC 2.5.8 asks 24 CSS px of a pointer target, and the kit holds every density to that floor with a test. A control's own `size="sm"` draws 4px under it, but a small button, action button or checkbox still accepts the pointer over the full control height: an invisible pseudo-element extends its hit area 2px past the drawn box on every side, so a toolbar of `sm` buttons keeps its 20px look and meets the criterion where the buttons sit at least 4px apart, which a `jx-toolbar` guarantees. A `jx-action-group` does not guarantee it: its default gap is 2px, so two extended neighbours overlap and the later one keeps the strip, which leaves every small member but the last at 22px on the packed axis; a compact group packs its members touching, and there their hit area on that axis is what they draw. A group of small buttons that must meet the criterion needs `md` members or a 4px gap of its own. The text field's clear button and the combobox's toggle carry the same kind of hit area at every field size, and the number field's steppers are covered by the input beside them, which takes the same step by arrow key. A closable tab's close button carries one too. Each of these extended hit areas is layout-contained, so none of them ever makes a scrolling panel scroll, and `contain: layout` is the value to use: `content` and `strict` also contain paint, which clips the hit area back to the drawn box. The same containment leaves a small button, action button or checkbox with no text baseline, so centre small controls in a flex row (`align-items: center`) instead of aligning them on `baseline`, or the text beside them drops to the button's bottom edge. Two things are outside the claim and the spec says so: a small text field, number field, select or combobox draws its native input 4px under the floor (a native input cannot carry a pseudo-element), so use `md` where the field must meet the criterion; and the split handle stays a 5px line, because a 24px strip would cover the panes it divides.
...
A rule on an ancestor reaches every matching part inside it, in every element. A text field, select or combobox always carries an empty `error` paragraph, so that the first refusal is announced when it arrives. A rule such as `& [part="error"]` that you meant for your own banner therefore lands on those paragraphs too: the fields keep the empty one from drawing, but once a field has a refusal to show, your rule restyles it. Name your own nodes something the kit does not use, such as `failure`, or anchor the rule with `>`.
...
The native `input` and `change` events bubble from the field as they always do. `invalid` marks a refused value and reaches the control as `aria-invalid`; `error` draws the sentence explaining it under the field and announces it as it changes; while there is no error the paragraph takes no space, draws nothing and cannot be clicked; `help` draws a sentence of guidance. Each sentence carries an id the control names in `aria-describedby`, the error first, so a reader who tabs away and back hears why the value was refused rather than only catching the announcement once. `labelledby` and `describedby` forward to the control as well, and your own `describedby` is read after the field's own two sentences. `type` is any text-like input type, `size` is `sm`, `md` or `lg`, and `mono` draws the value in the monospace face for a path, a selector or a colour. `name`, `autocomplete`, `disabled`, `readonly` and `required` forward to the control. The control carries the field's value as its default value too, so resetting a form the field sits in leaves the control saying what the field says rather than emptying it: the value is the field's own state, and putting a different value back is a write of `value` from the host. To focus the field and select its value from a host, call `selectValue(host, "all" | "stem" | "none")` or `focusField(host)` from `@jxsuite/ui/behaviors/textfield`.
...
`jx-switch` takes a `hint`, and it is the same hybrid a button's is: a real tooltip while the switch can be flipped, rendered as a `jx-tooltip` child of the element and described on the input, and the native `title` of the wrapping label while the switch is `disabled`, which still shows on hover and is where a reason such as "sign in to publish" belongs. One thing differs from a button, and the browser decides it rather than the kit: a label cannot carry `interestfor`, since only `a`, `area` and `button` support it, so the switch wires its tip by `for` alone and the kit binds the row itself on every browser, Chrome 152 included. Read a switch's hint in a test the way you read a button's: from the tip's text, or from `title` on the label only while it is disabled. Both take `label`, `labelledby`, `describedby`, `name`, `disabled` and `size`. The native `change` and `input` bubble from the control, so `event.target.checked` reads as it would from a bare input. Both have the parts `control`, `input` and `label`, and on these two `control` is the `<label>` wrapping the input rather than the input itself.
...
`selects` decides what the row is: `"none"` is a toolbar of separate actions, `"single"` is a set of choices where one wins, and `"multiple"` is a set of independent switches. Arrow keys move within the row and Tab leaves it, so a toolbar of ten buttons costs one tab stop rather than ten. `compact`, on by default, draws the buttons as one segmented control: a shared frame, a divider between buttons, and the chosen button highlighted (in the solid accent with `emphasized`), whatever each button's `quiet` says. Set `compact` to `false` for a row of separate, borderless tools. It reaches only the buttons that are the group's own children, so a nested group keeps its own seam. A compact row also stays on one line, because the seam is drawn from each button's position among its siblings and no rule can find the end of a wrapped line: a row whose choices may not fit is `compact: false` as well, with `quiet: false` on each button so every line reads the same. A disabled button in a compact row fades its label and keeps its share of the frame. `label` names the group, and a reader entering it hears the name; `orientation` is `horizontal` or `vertical`, and it picks the arrow pair and turns the row. The group has no parts.

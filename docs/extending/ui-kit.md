...
Studio's own chrome is a Jx project too: `packages/studio/project.json`, whose documents are the surfaces under `src/surfaces/`. Open either project and Studio edits itself. Save a component under `components/` and every part of the shell that draws that element is re-mounted with the new definition; save a surface under `src/surfaces/` and the roots drawn from it are re-mounted, keeping the host state they had. A toast says how many places changed, and a warning says why a save was not applied: a document whose `tagName` disagrees with its file name, or a surface file no adapter mounts. This works in the shell around you, not yet on the canvas: a stylebook page on the canvas draws the elements from the project's files, and an element with a behaviour sidecar does not mount there today, because the canvas frame cannot load `jx-ui:` modules. The lane acts only on those two paths, so saving any other project's files does nothing to the shell.

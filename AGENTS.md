...
- Every substantive spec edit is a release: run `bun run spec:change <spec.md> <major|minor|patch> -m "<what changed>"` to record it as a fragment under `specs/changes/` (minted on the release branch in merge order, so two pull requests releasing one spec never conflict), or `bun run spec:bump …` to advance the version, restamp `**Updated:**`, and add the `## Changelog` entry in place. CI blocks a changed spec body that wasn't released (`bun run docs:spec-release`). See ./specs/README.md.
...
- Implementation plans live in `plans/` (contract: `plans/README.md`) and are deleted when they land. Never cite a plan id or slice id outside `plans/`; cite the spec section. `bun run plans:check` enforces both, and `bun run plans:status` shows what is ready to work on.

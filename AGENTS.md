...
- Every substantive spec edit is a release: run `bun run spec:bump <spec.md> <major|minor|patch> -m "<what changed>"` to advance the version, restamp `**Updated:**`, and add a `## Changelog` entry. CI blocks a changed spec body that wasn't released (`bun run docs:spec-release`). See ./specs/README.md.

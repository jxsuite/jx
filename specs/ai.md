...
> **Status: Implemented.** `packages/ai/src/gateway/` (`@jxsuite/ai/gateway`), adopted by `packages/server/src/ai-api.ts`; `packages/server/tests/ai-upstream-fixtures.test.ts` and `packages/ai/tests/gateway-*.test.ts`.

The server half of the `ai/chat` and `ai/models` routes is **one implementation**, `@jxsuite/ai/gateway`, which any backend serving them can run. Before it, the proxy's wire work lived inside `@jxsuite/server`, and a second backend had to write its own copy, which is how two normalizers came to disagree. The extraction changes no byte a client receives: the server's recorded upstream fixtures produce the same frames through the gateway as they did through the proxy before it.

The gateway owns the **wire**:

- `createChatHandler(options)` answers the chat route, taking each request through a fixed order, where the first step that refuses answers the request as `application/problem+json` and nothing later runs: the host's `admit` (before the body is read), the host's `resolveUpstream`, reading the body (a `413` once it passes `maxBodyBytes`, measured on the bytes actually read; a `400` when it is not JSON), the `messages` array (`400` when it is present and not an array), `maxMessages` (`413`), the host's `onAccepted`, and then the stream. The version 1 body is forwarded as the OpenAI-compatible request it describes, and the response is always a `200 text/event-stream`: a failure after that point is an `error` frame, never a status (§2).
- `normalizeOpenAIStream(response)` is the stream normalizer, moved from the server verbatim, including the `usage` frame and reading past the finish until `[DONE]` or the end of the body (§2).
- `encodeSse(frames)` frames each event as one `data:` line and a blank line, and nothing else.
- `problemResponse(refusal)` renders a refusal. The body satisfies both kinds of reader: `error` accompanies `detail`, and a refusal's `code`, when it has one, is sent beside them.
- `modelsResponse(body)` answers the models route, always with `200`, because it is a capability probe (§2.1).
...
| --------------------------------------------------------------------------------------------------------- | ---------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [WHATWG HTML](https://html.spec.whatwg.org/)                                                              | **Subset** | §2    | packages/ai/src/gateway/sse.ts, packages/ai/tests/gateway-sse.test.ts, packages/server/src/ai-api.ts                                         | Server-Sent Events only. The proxy emits `text/event-stream` frames discriminated by a `type` field in the JSON payload; it sends no `retry:` field and no event ids, so a dropped stream is not resumable.                                                                                                                                                                                                                      |

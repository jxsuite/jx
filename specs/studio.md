...
**The Document Header card is Edit's alone, and it is docked above the page.** The card (the page's title, route, layout picker, frontmatter fields and the door to Search appearance) is drawn by the stage in Edit only, as a band across the pane at 1:1 standing above the page's scroller: its width is the pane's rather than the chosen breakpoint's or a dragged width, the page scrolls beneath it and is clipped at the card's own bottom edge with no dead strip between the two, and a long form scrolls inside the card — under a wheel as well as by its scrollbar, because the stage forwards a wheel to the page only when nothing under the pointer can still answer it. The card is capped at 40% of the WINDOW and at half the PANE, whichever is smaller: the window alone left a short pane (a laptop with the bottom dock open) showing a sliver of page. Design draws no card, so its artboards get the whole pane; the Navigator's Page panel carries the same fields in every mode. A lens draws none either, because two cards over one document's frontmatter would be two writers for one field. Geometry that used to measure the stage now measures the page's scroller, so a reveal centres a node in what is visible under the card, and the block action bar is clipped against the scroller and flips below its element rather than standing on the card when there is no room above it inside the scroller.
...
A record's `title` is the button's accessible name, its tooltip and the panel header. The text under the glyph is the title unless the record declares a shorter `railLabel`, which must be a leading part of the title (WCAG 2.5.3, Label in Name) so the words on screen are inside the name a reader hears; `registerPanel()` refuses one that is blank or is not. Source Control declares **Source**: the full title is 71px against the stacked label's 48px box at `--jx-text-xs`, and the kit ellipsizes a stacked label rather than wrapping it, so the rail read "Source …". The header, the `Show Source Control` command and the chrome-budget rows keep the title.
...
1. **Sync bar** — Refresh, the ahead/behind status with the last-updated time, then the Fetch/Pull/Push action group. With no remote it reads "Local only (no remote)" and offers **Create GitHub repository**.
2. **Active branch row** — the branch icon and name, then the branch picker (`jx-select`), which lists local branches plus "+ New branch...".
3. **Sub-tabs** — "Local Changes", carrying the changed-file count in its own label, and "History". They select which of the two views below fills the panel's one body; the strip ellipsizes rather than scrolling.
4. **Local Changes**, top to bottom: the **commit area** (a multi-line message field, `Ctrl+Enter` to commit, then a "Commit and sync" button with a caret menu whose entry is "Commit (don't sync)"); **Staged Changes**, a section with a file list and a per-file Unstage button, with Unstage All in its header; and **Changed Components**, the unstaged and untracked files, with per-file Stage and Discard buttons and Stage All in its header.
5. **History** — the recent commits (short hash, message, author, how long ago), or the sentence that says what a commit is before the first one exists.
6. **A busy line and a refusal**, at the foot, each drawn only while there is one. The refusal is `failure`, not `error`, for the reason `studio-ui-guidelines.md` §10 gives.

The sync bar and the Active branch row are wrapping rows (`studio-ui-guidelines.md` §4.6). Refresh and the status form one group, and so do the branch icon and name; neither group ever splits. The remote verbs (or Create GitHub repository) and the branch picker drop to a line of their own when the Navigator is too narrow to hold them beside that group, and a wrapped picker spans the row. Where the wrap happens is decided by the content's width (the status text, the clock format, the branch name), never by a breakpoint — so at the Navigator's default 240px the verbs are already on a line of their own under an en-US 12-hour clock, and so is the picker for any branch name of ordinary length. That is the intended reading of the rule rather than an edge of it: the alternative at that width is a status that breaks word by word beside three 20px icons. The panel's labelled buttons wrap their words, and its sub-tabs ellipsize, so nothing scrolls sideways at the Navigator's 160px floor.
...
| `"$input": "button-group"` | Row of kit action buttons, one per `$buttonValues` value (or `enum`), each drawing its `$icons` glyph (a kit manifest name) or, where it has none, the CSS keyword in full and in lower case, which is the only kind of button in a row that carries any text; the value is each button's name and tooltip, a second press clears, and values beyond the row open from a ··· menu |
...
A KEY on a record (`icon: "folder"`) is **not a tag**. It resolves through the kit's icon MANIFEST, and never reaches `customElements` at all. A panel record's key is drawn by the rail through `jx-icon`: a key with no glyph is not a missing element, it is zero nodes above the label and one console warning, and defining an element does nothing because the tag is never constructed. A css-meta `$icons` value (`data/css-meta.json`, the Style tab's button rows) is the same kind of key: it is drawn through `jx-action-button`'s `jx-icon` and misses just as silently. It is held to the manifest by `tests/metadata.test.ts` rather than by `check-icons.ts`, because it is data the tab reads rather than a literal on a record. It names the manifest glyph directly. The table that once translated css-meta's own names (`display-flex`, `justify-start`) resolved every name it lacked to no glyph, and the whole Display row shipped as abbreviations.
...
3.  **A blank field never deletes.** Storing an empty value and forgetting a value are different operations, and only the second one forgets. Conflating them read as a convenience until a form blanked its own drafts on Save while the sheet stayed open: pressing Save a second time on the emptied form revoked the credentials the first press had stored. A form must therefore also show what it saved rather than clearing itself, and clearing is the Accounts section's Disconnect. A form offers Save and Cancel only while its drafts differ from what is stored, compared as the store would store them (trimmed, an endpoint without its trailing slash). At rest there is nothing to keep and nothing to abandon, so neither is drawn, and answering either moves focus to the form's first field rather than letting it fall out of the sheet with the button that was pressed. A draft the reader has not touched is a view of the store, so a credential changed or revoked elsewhere (a Disconnect in Accounts, another window) reaches it on the next repaint rather than surfacing as an unsaved edit that Save would write back.
...
The code half draws no zoom pod. It is one editor filling the stage with no pan/zoom surface under it, so nothing there zooms, and a pod would sit on the editor's own scrollbar and overview ruler. The pod comes back with the visual half, and the switch between the halves redraws the pane's chrome as well as its stage.

...
**The Document Header card is Edit's alone, and it is docked above the page.** The card (the page's title, route, layout picker, frontmatter fields and the door to Search appearance) is drawn by the stage in Edit only, as a band across the pane at 1:1 standing above the page's scroller: its width is the pane's rather than the chosen breakpoint's or a dragged width, the page scrolls beneath it, and a long form scrolls inside the card, capped at 40% of the window height. Design draws no card, so its artboards get the whole pane; the Navigator's Page panel carries the same fields in every mode. A lens draws none either, because two cards over one document's frontmatter would be two writers for one field. Geometry that used to measure the stage now measures the page's scroller, so a reveal centres a node in what is visible under the card and the block action bar is clipped against the scroller rather than floating over the card.
...
| `"$input": "button-group"` | Row of kit action buttons, one per `$buttonValues` value (or `enum`), each drawing its `$icons` glyph (a kit manifest name) or the value's abbreviation where it has none; the value is each button's name and tooltip, a second press clears, and values beyond the row open from a ··· menu |

...
`isModalOpen()` reports whether a surface with an underlay is up — a `jx-dialog` in either the dialog or the modal layer, and still the two Spectrum selectors beside them, kept deliberately: the question is about the live DOM, so it costs nothing to keep answering it for an element an extension could put in a layer, and narrowing it would be a silent narrowing of a safety rule. It is derived from the live DOM, not a registration counter, so the rule is simply _whatever blocks the mouse blocks the keyboard_: the app-level keydown handlers (`editor/shortcuts.ts`, `panels/block-action-bar.ts`) return early while it is true. Without that gate, <kbd>Delete</kbd>, <kbd>Enter</kbd>, ⌘S, ⌘W and ⌘Z keep driving the document behind a surface the author cannot even click on.

...
- On open the surface moves focus into itself — the first enabled focusable in the body, else the dialog's own cancel button (the footer is drawn cancel → secondary → confirm, so the first button is the least destructive landing spot), else the dialog itself, so a body of static content (a progress spinner) still receives keys. A body that already claimed focus keeps it: `showPromptDialog`'s field, and a sheet with a section list claims the item for the section it OPENED on rather than the first one. Preferences opens on any of its four, and taking the platform's default left the ring on Appearance while `aria-current` and the selected fill were elsewhere — two rows reading as active at once, with the keyboard on the wrong one.
...
**A surface opts back in only on a named content part**, from its own surface document: `userSelect: "text"`, or `"all"` for a token that is only useful whole. The set is the dialog message `[part="message"]` and the island's own prose (`surfaces/dialog.json`, §8.7), the GitHub device code (`surfaces/github-auth.json`, `all`), About's values and package rows (`meta-value` and `package-row` in `surfaces/about.json`, the version, build date, commit and resolved `@jxsuite/*` numbers a bug report is pasted from, which no copy action offers), the Assistant's transcript bodies (`user-body`, `md`, `streaming` and `import-log` in `surfaces/ai-chat.json`), and the captured logs in Problems (`detail`) and Activity (`log`).

**An opt-in names a part, never a mount point.** `user-select` inherits, so a rule on a box that hosts other documents reopens selection inside all of them. `[part="island"]` is both the message's richer form and where the Open Grid picker and Push Schema mount whole surfaces, so the opt-in there names the text-level children a lit body writes (`> p`, `> span`, `> strong`, `> em`, `> code`) rather than the box: a mounted document roots a `div`, so its headings, rows and buttons stay unselectable like the rest of the interface.
...
- [ ] A surface MOUNTED in another one qualifies any part name its host also writes rules for, and a host writes a rule that cannot reach its guest — a child of a box the host draws, or an attribute only the host's node carries. An island is a box in the host's own light DOM, so `[part="failure"]` in `new-project.json` reached the managed-connect refusal mounted inside it (now `connect-failure`), and Preferences' Keyboard-row and section-heading rules reached the credentials form's key field and title. The same test holds each island host against the documents it hosts

...
Generated by `bun run docs:generate`, a gitignored build output the site writes before it builds and never a committed file. It carries the conformance table across all specs, the tracked gap list by tier, the declined standards with their reasons, the adoption backlog (§11), and the catalog itself — which is the page where a human can see a wrong identifier.
